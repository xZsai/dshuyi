...
# 如果url中需要中文，则需要通过quote 进行转义，不然会报错
...
for job in jobitem:
    # 用来存放单条招聘信息
    job_dic = {}
    # get_text() 用来获取所有文本内容，但是是以字符串列表的方式显示
...
cuosor = db.cursor()
for job in job_list:
    sql = "insert into job values(null,'%s','%s','%s','%s','%s','%s')" % (job["job_name"]
                                    ,job["job_sal"],job["company_name"],job["job_place"],
